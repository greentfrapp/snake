...
from curr_env_d import SnakeEnv
...
def play(env, model, n_steps=500, bound=None):
    obs = env.reset(bound=bound)
    total_reward = 0
...
if __name__ == "__main__":
    ...
    for size in range(4, 5):
        for n in range(1):
            ...
            log_file = open(logs_folder / f"log_S_{size}_{n}_curr.csv", 'w')
            log_file.write("iteration,avg score,max avg,max single\n")

            # Instantiate the env
            # size = 20
            curriculum = 0.5
            env = SnakeEnv(size, curriculum=curriculum)
            # wrap it
            ...
            while True:
                ...
                env.curriculum = 0
                for _ in range(10):
                    scores.append(play(env, model, bound=size))
                ...
                if np.max(scores) == 10 * size ** 2 + 70:
                    model.save(models_folder / f"model_S_{size}_{n}_curr_last")
                    print("Max score achieved, saving model...")
                    break
                elif mean_score >= highest_reward:
                    plateau = 0
                    model.save(models_folder / f"model_S_{size}_{n}_curr")
                    if mean_score > highest_reward:
                        curriculum *= 0.975
                        env.curriculum = curriculum
                        print(f"Raising curriculum to {curriculum}...")
